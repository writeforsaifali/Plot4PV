...
def extract_variation(file_name: str) -> str:
    ...
    This helper identifies the variation portion of a PV data file
    name.  The naming schemes used by PV laboratories often embed
    the device number and pixel identifier at the end of the file
    name, separated from the variation by a separator such as
    underscore, hyphen or space.  For instance::

        0001_2026-02-26_15.29.07_Stability (JV)_Z_STPVK1SAM5000MGF_13-1A.txt

    here the variation is ``Z_STPVK1SAM5000MGF``; ``13`` is the device
    number and ``1A`` is the pixel.  In another example::

        0001_2026-02-26_15.29.07_Stability (JV)_Z_ST-PVK1=SAM-5000-MGF-13-1A.txt

    the variation is ``Z_ST-PVK1=SAM-5000-MGF``.  The key property is
    that the device portion comprises digits immediately preceding the
    final hyphen‑delimited pixel, and that this digit sequence is
    separated from the variation by at least one underscore, hyphen or
    space.  This function leverages :func:`parse_variation_device_pixel`
    to robustly extract the variation and then trims any trailing
    separators.
    ...
    -------
    str
        The inferred variation identifier.  If the input is blank or
        cannot be parsed, an empty string is returned.
    """
    if not file_name:
        return ""
    # Use the JV parsing helper to obtain the variation, device and pixel
    try:
        variation, device, pixel = parse_variation_device_pixel(file_name)
        # parse_variation_device_pixel returns a tuple where variation may
        # contain trailing separators; strip them off for a clean label
        if variation:
            # Remove trailing underscores, hyphens and spaces
            return variation.rstrip("_- ")
    except Exception:
        pass
    # Fallback: replicate original logic for safety
...
def parse_variation_device_pixel(file_name: str) -> Tuple[str, Optional[str], Optional[str]]:
    """
    Extract variation, device number and pixel identifier from a PV data file name.

    PV file names typically embed these identifiers after the study type
    information (e.g. ``Stability (JV)``) and before the file extension.  The
    variation name may include underscores, hyphens, spaces or other
    characters.  The device number consists of a sequence of digits
    immediately preceding the pixel identifier, separated by at least one
    underscore, hyphen or space.  The pixel identifier follows the final
    hyphen in the file name.  For example::

        0001_2026-02-26_15.29.07_Stability (JV)_Z_STPVK1SAM5000MGF_13-1A.txt

    yields variation ``Z_STPVK1SAM5000MGF``, device ``13`` and pixel ``1A``;

        0001_2026-02-26_15.29.07_Stability (JV)_Z_ST-PVK1=SAM-5000-MGF-13-1A.txt

    yields variation ``Z_ST-PVK1=SAM-5000-MGF``;

        0001_2026-02-26_15.29.07_Stability (JV)_Z ST PVK1 SAM 5000 MGF 13-1A.txt

    yields variation ``Z ST PVK1 SAM 5000 MGF``.

    The function first isolates the portion of the file name after the
    closing parenthesis of the study type (``)_``) as the variation-device-pixel
    segment.  It then separates the pixel by splitting on the last hyphen and
    uses a regular expression to identify the trailing digits (device) preceded
    by a separator (underscore, hyphen or space).  If no such pattern is
    found, the entire segment (minus the pixel) is returned as the variation.

    Parameters
    ----------
    file_name : str
        Name or path of the PV data file.
    ...
    stem = base.rsplit(".", 1)[0] if "." in base else base
    # Attempt to isolate the variation+device+pixel portion by locating the
    # closing parenthesis of the study type followed by an underscore.
    idx = stem.rfind(")_")
    if idx != -1:
        var_dev_pix_sub = stem[idx + 2:]
    else:
        # Fallback: use the last two underscore-delimited segments if available
        parts = stem.split("_")
        if len(parts) >= 2:
            var_dev_pix_sub = "_".join(parts[-2:])
        else:
            var_dev_pix_sub = stem
    # If no hyphen exists, there is no pixel identifier; treat the entire
    # substring as the variation and return early.
    if "-" not in var_dev_pix_sub:
        return var_dev_pix_sub.rstrip("_- "), None, None
    # Separate the pixel from the left-hand side by splitting on the last hyphen.
    var_dev, pixel = var_dev_pix_sub.rsplit("-", 1)
    # Use a regex to find the last occurrence of a separator (underscore,
    # hyphen or space) followed by one or more digits at the end of var_dev.
    import re  # Import locally to avoid polluting the global namespace
    m = re.search(r"([_\-\s])(\d+)$", var_dev)
    if m:
        sep_index = m.start(1)
        device = m.group(2)
        # Variation is everything before the separator
        variation = var_dev[:sep_index]
        variation = variation.rstrip("_- ")
        return variation, device, pixel
    # No pattern found; treat the entire var_dev as the variation
    variation = var_dev.rstrip("_- ")
    return variation, None, pixel
...
def main() -> None:
    ...
    if mode == "Box Plot":
        ...
        with st.expander("Summary Statistics", expanded=False):
            ...
            # -----------------------------------------------------------------
            # Additional table: maximum PCE and its associated PV parameters
            #
            # For each selected variation, compute the row in ``data`` with the
            # highest corrected PCE (``PCE_corrected``) and extract the
            # corresponding PV parameters (Voc, Jsc_corrected, FF) along with
            # the device and pixel identifiers from the file name.  Present
            # these records in a concise table so that users can quickly
            # identify the best‑performing devices per variation.
            pce_rows: List[Dict[str, Any]] = []
            for v in selected_variations:
                dfv = data[data["Variation"] == v]
                if dfv.empty or "PCE_corrected" not in dfv.columns:
                    continue
                try:
                    idx_max_pce = dfv["PCE_corrected"].idxmax()
                except ValueError:
                    continue
                row_data = dfv.loc[idx_max_pce]
                fname_pce = str(row_data.get("File Name", ""))
                _, dev_pce, pix_pce = parse_variation_device_pixel(fname_pce)
                scan_pce = parse_scan_number(fname_pce)
                raw_pixel = pix_pce or ""
                if raw_pixel and scan_pce:
                    display_pixel = f"{raw_pixel} ({scan_pce})"
                elif raw_pixel:
                    display_pixel = raw_pixel
                elif scan_pce:
                    display_pixel = f"({scan_pce})"
                else:
                    display_pixel = ""
                pce_row = {
                    "Variation": v,
                    "PCE_max": row_data.get("PCE_corrected"),
                    "Voc": row_data.get("Voc"),
                    "Jsc_corr": row_data.get("Jsc_corrected"),
                    "FF": row_data.get("FF"),
                    "Device": dev_pce or "",
                    "Pixel": display_pixel,
                }
                pce_rows.append(pce_row)
            if pce_rows:
                pce_df = pd.DataFrame(pce_rows)
                desired_cols = ["Variation", "PCE_max", "Voc", "Jsc_corr", "FF", "Device", "Pixel"]
                pce_df = pce_df[[c for c in desired_cols if c in pce_df.columns]]
                st.markdown("### Maximum PCE per Variation")
                st.dataframe(pce_df)
            ...
            for r in best_rows:
                ...
                if "Voc_max_device" in r:
                    dev_raw = r.get("Voc_max_device")
                    pix_raw = r.get("Voc_max_pixel_raw") if "Voc_max_pixel_raw" in r else r.get("Voc_max_pixel")
                    scan_raw = r.get("Voc_max_scan")
                    bd['voc'] = (dev_raw, pix_raw, scan_raw)
                if "FF_max_device" in r:
                    dev_raw = r.get("FF_max_device")
                    pix_raw = r.get("FF_max_pixel_raw") if "FF_max_pixel_raw" in r else r.get("FF_max_pixel")
                    scan_raw = r.get("FF_max_scan")
                    bd['ff'] = (dev_raw, pix_raw, scan_raw)
                if bd:
                    best_defaults[v] = bd
            if best_defaults:
        ...
        # Metric selection for overlay: choose which best JV curves to display.
        # When a user selects a metric here (PCE, Voc, FF or Jsc), the overlay
        # will include only the device/pixel corresponding to the maximum value
        # of that metric for each variation.  This selection is stored in
        # ``st.session_state['jv_overlay_best_metric']`` for use in the
        # include_flags computation below.
        overlay_metric_display_map = {"PCE": "pce", "Voc": "voc", "FF": "ff", "Jsc": "jsc"}
        _overlay_metric_label = st.sidebar.selectbox(
            "Overlay JV curves by best metric",
            list(overlay_metric_display_map.keys()),
            index=0,
            key="jv_overlay_best_metric"
        )
        # Labels, colours & shapes collapsed by default
        labels: List[str] = []
        colours: List[str] = []
        markers: List[str] = []
        # Wrap the label, colour and marker selection in a collapsed sidebar expander
        with st.sidebar.expander("Labels, Colours & Shapes", expanded=False):
        ...
        # Build a map of variations to devices and pixel entries so users can
        # quickly include or exclude entire variations from the JV overlay.
        variation_map_selection: Dict[str, Dict[str, List[Dict[str, Any]]]] = {}
        for idx, (fbuf, df) in enumerate(zip(jv_files, curves)):
            file_name = getattr(fbuf, "name", "")
            vsel, dsel, psel = parse_variation_device_pixel(file_name)
            dsel = dsel or ""
            psel = psel or f"File{idx + 1}"
            scan_sel = parse_scan_number(file_name) or ""
            if vsel not in variation_map_selection:
                variation_map_selection[vsel] = {}
            if dsel not in variation_map_selection[vsel]:
                variation_map_selection[vsel][dsel] = []
            variation_map_selection[vsel][dsel].append({"pixel": psel, "df": df, "index": idx, "scan": scan_sel})
        # Display variation-level checkboxes outside of the parsed data section.
        st.subheader("Select variations for JV overlay")
        best_defaults_sel = st.session_state.get('best_jv_defaults', {})
        for vname in sorted(variation_map_selection.keys()):
            var_key = f"jv_overlay_var_include_{vname.replace(' ', '_')}"
            # Use existing state if available; otherwise default to True only when best defaults exist for this variation
            if var_key in st.session_state:
                var_default_sel = st.session_state[var_key]
            else:
                var_default_sel = True if (vname in best_defaults_sel and best_defaults_sel[vname]) else False
            st.checkbox(
                f"Include variation {vname}",
                value=var_default_sel,
                key=var_key
            )
        ...
        with st.expander("JV Data", expanded=False):
            ...
            # Determine which metric is currently selected for overlay to highlight the best entry
            best_defaults = st.session_state.get('best_jv_defaults', {})
            metric_map_for_highlight = {"PCE": "pce", "Voc": "voc", "FF": "ff", "Jsc": "jsc"}
            selected_metric_label_highlight = st.session_state.get("jv_overlay_best_metric", "PCE")
            highlight_metric_key = metric_map_for_highlight.get(selected_metric_label_highlight, None)
            # Display each variation in its own collapsible expander
            for vname, dev_dict in variation_map_preview.items():
                with st.expander(f"Variation: {vname}", expanded=False):
                    for dname, entries in dev_dict.items():
                        ...
                        for entry in entries:
                            ...
                            # Determine if this entry corresponds to the best device/pixel for the selected metric
                            highlight = False
                            if highlight_metric_key and vname in best_defaults and highlight_metric_key in best_defaults[vname]:
                                metric_tuple = best_defaults[vname][highlight_metric_key]
                                dev_best = pix_best = scan_best = None
                                if metric_tuple is not None:
                                    if len(metric_tuple) >= 3:
                                        dev_best, pix_best, scan_best = metric_tuple
                                    elif len(metric_tuple) == 2:
                                        dev_best, pix_best = metric_tuple
                                if (dname == (dev_best or "")) and (pixel == (pix_best or "")):
                                    if (scan_best is None) or (scan_best == scan_entry):
                                        highlight = True
                            # Determine default for pixel checkbox: preselect only the highlighted entry
                            pix_key = f"jv_overlay_include_{idx}"
                            if pix_key in st.session_state:
                                pix_default = st.session_state[pix_key]
                            else:
                                pix_default = True if highlight else False
                            # Append a star to indicate the best entry
                            display_name_display = f"{display_name} ⭐" if highlight else display_name
                            st.checkbox(
        ...
        # Show checkboxes for including forward and reverse scans.  By default,
        # only forward scans are included because the user typically wants to
        # display a single scan for each JV curve (usually forward) when
        # overlaying the best metric.  Users may optionally enable reverse
        # scans via the second checkbox.
        include_fw = st.checkbox(
            "Include Forward Scans (FW)", value=True, key="jv_include_fw"
        )
        include_rv = st.checkbox(
            "Include Reverse Scans (RV)", value=False, key="jv_include_rv"
        )
        ...
        # Determine which metric's best curve should be used for each variation.
        # The sidebar selection (stored in 'jv_overlay_best_metric') returns one of
        # "PCE", "Voc", "FF" or "Jsc".  Map this to the internal key used in
        # ``best_jv_defaults``.
        metric_display_map = {"PCE": "pce", "Voc": "voc", "FF": "ff", "Jsc": "jsc"}
        selected_best_metric_label = st.session_state.get("jv_overlay_best_metric", "PCE")
        overlay_metric_key = metric_display_map.get(selected_best_metric_label, None)
        best_defaults = st.session_state.get('best_jv_defaults', {})
        for i in range(len(curves)):
            # Determine if the current curve's variation is selected
            ...
            if var_name is not None:
                ...
                var_flag = True
            pix_flag = False
            if overlay_metric_key is not None:
                # Automatically include only the best JV curve for the selected metric
                file_name_i = getattr(jv_files[i], "name", "")
                var_i, dev_i, pix_i = parse_variation_device_pixel(file_name_i)
                dev_i = dev_i or ""
                pix_i = pix_i or ""
                scan_i = parse_scan_number(file_name_i) or ""
                if var_i and var_i in best_defaults and overlay_metric_key in best_defaults[var_i]:
                    metric_tuple = best_defaults[var_i][overlay_metric_key]
                    dev_best = pix_best = scan_best = None
                    if metric_tuple is not None:
                        if len(metric_tuple) >= 3:
                            dev_best, pix_best, scan_best = metric_tuple
                            ...
                            dev_best, pix_best = metric_tuple
                    # Compare device, pixel and scan (when provided) to determine match
                    if (dev_i == (dev_best or "")) and (pix_i == (pix_best or "")):
                        if (scan_best is None) or (scan_best == scan_i):
                            pix_flag = True
            else:
                # Fallback: use per‑pixel checkboxes
